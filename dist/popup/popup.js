...
var PopupController = class {
  ...
  bindEvents() {
    ...
    globalThis.SV_DEBUG = {
      getState: () => this.sendMessage("GET_STATE").then((s) => {
        console.table(s);
        return s;
      }),
      getLogs: () => this.sendMessage("GET_LOGS").then((r) => {
        console.table(r.logs);
        return r.logs;
      }),
      dumpLogs: () => this.sendMessage("GET_LOGS").then((r) => r.logs?.forEach((l) => console.log(`[${l.level}][${l.module}] ${l.action}`, l.data || ""))),
      reset: () => chrome.storage.local.clear().then(() => location.reload()),
      startMock: () => {
        console.log("[SV_DEBUG] Connecting mock account...");
        return globalThis.SV_DEBUG.getState();
      }
    };
  ...
  async startRecording(target) {
    ...
    if (target.type === "screen" || target.type === "window") {
      ...
      console.log("[Popup] Tab target \u2014 skipping chooseDesktopMedia, using tabCapture", { tabId: target.tabId });
