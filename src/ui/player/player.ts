// ScreenVault — Player Script
// Loads the actual video buffer from the Service Worker for playback
...
async function initPlayer() {
    ...
    chrome.runtime.sendMessage({ type: 'GET_RECORDING_BUFFER' }, (response) => {
        if (response?.buffer && response.buffer.byteLength > 100) {
            ...
            const errorMsg = response?.buffer 
                ? `Recording is too small (${response.buffer.byteLength} bytes). It likely contains no data.`
                : "No recording found in memory. Did you close the popup before it saved?";
